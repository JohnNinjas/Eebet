...
class GalleryBehavior extends Behavior
{
     ...
     * @param $path
     */
    public function replaceImage($imageId, $path, $gal)
    {
        $this->createFolders($this->getFilePath($imageId, 'original'));

        $originalImage = Image::getImagine()->open($path);

        $gallery = Image::getImagine()->open($gal);
        //save image in original size

        $gallery->save($this->getFilePath($imageId, 'full_size_orig'),[]);
        ...
        foreach ($this->versions as $version => $fn) {
            ...
            if ($version == 'gal')
                $image = call_user_func($fn, $gallery);
            else
    ...
    public function addImage($file)
    {
        try {
            $src = $file->tempName;
            $dst = \Yii::getAlias('@frontend') . '/web/images/tmp/' . $file->name;
            ...
            if ($width > $height) {
                $newWidth = $width;
                $newHeight = ($width / 10) * 7;
            } else {
                $newWidth = ($height / 7) * 10;
                $newHeight = $height;
            }
            ...
            $color = imagecolorallocate($new, 255, 255, 255); //Создание цвета для изображения
            imagefill($new, 0, 0, $color);
            imagecopyresized($new, $img, $nx, $ny, 0, 0, $nw, $nh, $width, $height);
            ...
            $this->replaceImage($id, $dst, $file->tempName);
